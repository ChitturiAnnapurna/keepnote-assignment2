...
<body>
	...
	<table border="1">
		...
		<c:forEach var="notesdetails" items="${notes}">
			<tr>
				<td>${notesdetails.noteId}</td>
				<td>${notesdetails.noteTitle}</td>
				<td>${notesdetails.noteContent}</td>
				<td>${notesdetails.noteStatus}</td>
				<td>${notesdetails.createdAt}</td>
					<td><form action="updateNote" method="post">
						<input type="hidden" id="noteId" name="noteId"
							value="${notesdetails.noteId}" />
